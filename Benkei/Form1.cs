    ...
    public partial class Form1 : Form
    {
        ...
        private void OnLoaded(object sender, EventArgs e)
        {
            try
            {
                ...
                var imeOnRules = loader.Load(configPath);
                Log($"IME ONルール読み込み完了: {imeOnRules.Count}件");
                var imeOffRules = loader.Load(alphabetConfigPath);
                Log($"IME OFFルール読み込み完了: {imeOffRules.Count}件");
                var imeOnEngine = new NaginataEngine(imeOnRules);
                var imeOffEngine = new NaginataEngine(imeOffRules);
                _interceptor = new KeyboardInterceptor(imeOnEngine, imeOffEngine, OnConversionStateChanged);
                _interceptor.Start();
